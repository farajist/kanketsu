...
describe('url.service', () => {
  ...
  it('should find stored url', async () => {
    const hash = await urlCache.findUrl(
  ...
  it('should return null when no hash is found', async () => {
    const nullHash = await urlCache.findUrl('https://404.yeah');
  ...
  it('should generate a unique ID and update count', async () => {
    ...
    const uid = await urlCache.uniqId();
  ...
  it('should correctly save a new url', async () => {
    ...
    const payload = await urlCache.set({
      url: newLongUrl,
      startDate: new Date(),
  ...
  it('should get longUrl with meta from short version', async () => {
    const longUrlWithMeta = await urlCache.get('UV5RkzXh');
  ...
  it('should throw on non-existing shortUrl hash', async () => {
    await expect(urlCache.get('f4fOf4')).rejects.toThrowError(
      /hash not found/
    );
  });
